...
func (d DigitArray) Get(index int) (int8, error) {
	...
	if index == 0 {
		if d.IsLeadDigitOverflowing() {
			return d.FixLeadDigit(), nil
		} else {
			return d[index], nil
		}
	}
...
func (d DigitArray) CollectOverflowFromLeadDigit() int8 {
	initialLeadDigit := d[0]
	d[0] = d.FixLeadDigit()
	return (initialLeadDigit / 10)
}

func (d DigitArray) FixLeadDigit() int8 {
	leadDigit := d[0]
	if leadDigit < 0 { // LeadDigit is Negative
		return (10 - ((-leadDigit) % 10))
	}
	return (leadDigit % 10)
}
...
func (d DigitArray) Minus(other DigitArray) DigitArray {
	...
	for i := newSize - 1; i >= 0; i-- {
		...
		if diff < 0 {
			borrowIndex, err := findBorrowableIndex(i-1, result)
			if err == nil {
				result[borrowIndex]--
...
func (d DigitArray) FindBorrowableIndex(startIndex int) (int, error) {
	return findBorrowableIndex(startIndex, d)
}
...
func findBorrowableIndex(startIndex int, digitArray []int8) (int, error) {
	for i := startIndex; i >= 0; i-- {
		if digitArray[i] > 0 {
			return i, nil
		}
	}
	return -1, errors.New("no index found")
}
